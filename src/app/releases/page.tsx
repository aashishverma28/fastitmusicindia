...
export default function ReleasesPage() {
  ...
  const handleEditClick = (e: React.MouseEvent, rel: any) => {
    e.preventDefault();
    e.stopPropagation();
    setEditingRelease({
      id: rel.id,
      ...
      youtubeUrl: rel.youtubeUrl || rel.audioUrl || "",
      coverArtUrl: rel.cover || rel.coverArtUrl || "",
      genre: rel.genre || "Pop",
      releaseDate: rel.releaseDate ? new Date(rel.releaseDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      slug: rel.slug || "",
  ...
  const handleEditReleaseSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      console.error("Error editing release:", err);
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleEditArtworkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setEditingRelease(prev => ({ ...prev, coverArtUrl: url }));
    } catch (err: any) {
    ...
    <div className="min-h-screen pt-24 pb-20 px-8 relative overflow-visible">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="absolute top-10 right-20 hidden lg:block z-20">
          <BadgeStamp text="Real Sounds" type="pink" />
        </div>
        ...
        {/* Catalog Grid (with extra gap spacing on right to accommodate vinyl record slideout) */}
        {isLoading ? (
          ...
          <motion.div 
            ...
            initial="hidden"
            animate="visible"
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-14 gap-y-14"
          >
            {filteredReleases.map((rel: any) => (
              <motion.div key={rel.id} variants={itemVariants}>
                <VinylCard
                  ...
                  title={rel.title}
                  artist={rel.artist}
                  cover={rel.cover || rel.coverArtUrl || "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=800&q=80"}
                  slug={rel.slug || rel.id}
                  genre={rel.genre}
                  audioUrl={rel.audioUrl}
                  onPlay={(e) => handlePlay(e, rel)}
                  isAdminOrStaff={isAdminOrStaff}
                  onRemove={(e) => handleRemoveRelease(e, rel.id)}
                  onEdit={(e) => handleEditClick(e, rel)}
                />
              </motion.div>
            ))}
          </motion.div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="relative group aspect-square bg-[var(--background)] border-2 border-[var(--foreground)] rounded-none overflow-hidden flex flex-col items-center justify-center p-4">
                        ...
                        <input 
                          ...
                          accept="image/*"
                          className="absolute inset-0 opacity-0 cursor-pointer z-20"
                          onChange={handleEditArtworkUpload}
                          disabled={isUploadingArtwork}
                        />
                        {isUploadingArtwork && (
                          <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-35">
